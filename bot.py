#!/usr/bin/env python3
"""
Manhwa Downloader Bot - Multisitio (Configurable desde Telegram)
"""
    ...
    try:
        ...
        if not site_config:
            edit_message(chat_id, message_id, 
                "❌ Sitio no soportado\n\n"
                "Usa /sites para ver los sitios disponibles."
            )
        ...
        # Mensaje mejorado con código para copiar fácilmente
        send_message(
            chat_id,
            f"✅ *{title}*\n"
            ...
            f"💡 _Mantén presionado el enlace para copiarlo_"
        )
...
def list_sites(chat_id):
    if not SITES:
        send_message(chat_id, "📋 No hay sitios configurados.\n\nUsa /addsite para agregar uno.")
        return
    ...
    try:
        ...
        if len(parts) < 3:
            send_message(chat_id, 
                ...
                "Ejemplo:\n"
                "`/addsite Rncalation | rncalation.online | div.page-img-wrap img.page-img | title | div.page-img-wrap img.page-img`\n\n"
                "📌 *Explicación:*\n"
                "• nombre: Nombre del sitio\n"
                "• url_pattern: Texto que identifica la URL (ej: rncalation.online)\n"
                "• image_selector: Selector CSS para las imágenes\n"
                "• title_selector: (opcional) Selector para el título (por defecto: title)\n"
                "• scroll_selector: (opcional) Selector para contar imágenes durante el scroll"
            )
            return
        ...
        if 'text' in msg:
            ...
            if is_admin(chat_id):
                ...
                if text.startswith('/addsite'):
                    ...
                    if args:
                        ...
                        send_message(chat_id, "❌ Especifica los parámetros.\nUsa: `/addsite nombre | url | selector`")
                    return 'ok'
                ...
                if text.startswith('/removesite'):
                    ...
                    if key:
                        ...
                        send_message(chat_id, "❌ Especifica el ID del sitio.\nUsa: `/removesite rncalation`")
                    return 'ok'
                ...
                if text.startswith('/togglesite'):
                    ...
                    if key:
                        ...
                        send_message(chat_id, "❌ Especifica el ID del sitio.\nUsa: `/togglesite rncalation`")
                    return 'ok'
            
            # Comando start (público)
            if text == '/start':
                sites_list = "\n".join([f"• {s['name']}" for s in SITES.values() if s.get('active', True)]) if SITES else "No hay sitios configurados aún."
                send_message(chat_id, 
                    ...
                    f"💡 _Mantén presionado cualquier texto para copiarlo_"
                )
                return 'ok'
            ...
            # Procesar URL
                ...
                if not site_config:
                    ...
                    send_message(chat_id, 
                        f"❌ Sitio no soportado\n\n"
                        f"Sitios disponibles:\n{sites_list}"
                    )
                    return 'ok'
...
if __name__ == '__main__':
    # Manejo seguro del puerto
    try:
        port = int(os.environ.get("PORT", "10000"))
    except (ValueError, TypeError):
        port = 10000
        logger.warning("⚠️ Puerto no válido, usando 10000 por defecto")
    ...
    if SITES:
        ...
        print("   ⚠️ No hay sitios configurados")
        print("   Usa /addsite para agregar uno")
    print("\n📋 Comandos de admin:")
